...
const REWRITE_STORAGE = 'REWRITE_STORAGE';
...
export default function favoriteStorage(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case REMOVE_GROUP_ITEM :
            return {
                ...state,
                groups: state.groups.filter((group, index) => group !== action.item &&
                    state.groups.indexOf(group) === index),
            };
        case ADD_AUD_ITEM :
        ...
        case REMOVE_AUD_ITEM :
            return {
                ...state,
                auditoryies: state.auditoryies.filter(
                    auditory => auditory !== action.item),
            };
